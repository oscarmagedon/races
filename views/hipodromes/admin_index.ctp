...
<tr>
	<th>Filtrar Por:</th>
	<td><?php 
		echo $form->input('nat',array('options' => array(
            2 => 'Todas', 0 => 'Internacionales', 1=>'Nacionales'),
            'value'=>$nat,'label'=>"Tipo",
            'class'=>'filter_input','style'=>'width:120px'))
	?></td>
	<td><?php 
		echo $form->input('name',array(
            'value'=>$name, 'label'=>"Por Nombre...",'class'=>'filter_input'))
	?></td>
...
<tr>
    ...
    <th><?php echo $paginator->sort('tvgnick');?></th>
    <th><?php echo $paginator->sort('bovada');?></th>
    <th><?php echo $paginator->sort('BOV 2','bovalt');?></th>
    <th><?php echo $paginator->sort('GMT','htgmt');?></th>
	<th><?php echo $paginator->sort('NAC/INT','national');?></th>
	<th><?php echo $paginator->sort('CLASE','class');?></th>
...
?>
	<tr<?php echo $discls ?>>
        ...
        <td>
			<?php echo $hipodrome['Hipodrome']['tvgnick']; ?>
		</td>
        <td>
			<?php echo $hipodrome['Hipodrome']['bovada']; ?>
		</td>
        <td>
			<?php echo $hipodrome['Hipodrome']['bovalt']; ?>
		</td>
		<td>
        ...
        <td>
			<?php echo $hipodrome['Hipodrome']['class']; ?>
		</td>
		<td class="actions">
...
$(function() {
    ...
    $("#filt").click(function() {
        var urlh = '<?php echo $html->url(array('action' => 'index/')) ?>',
            nat  = $('#nat option:selected').val(),
            name = $('#name').val();
            
        location = urlh + '/' + nat + '/' + name;
    });
});	
</script>