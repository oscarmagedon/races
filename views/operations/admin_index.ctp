...
$(function(){
	...
	$("#filt").button({ icons: {primary: "ui-icon-zoomin"}}).css('font-size','140%').click(function(){
		...
		var us = $("#user").val();
		var ty = $("#type").val();
		location = '<?php echo $html->url(array("action"=>"index","/")) ?>'+'/'+da+'/'+ho+'/'+us+'/'+ty;
	});
});	
</script>
...
<div class="paging">
	<?php echo $paginator->prev('<< '.__('previous', true), array('url'=>array('action'=>'index',$date,$hour,$profile_id,$optype_id)), null, array('class'=>'disabled'));?>
 | 	<?php echo $paginator->numbers(array('url'=>array('action'=>'index',$date,$hour,$profile_id,$optype_id)));?>
	<?php echo $paginator->next(__('next', true).' >>', array('url'=>array('action'=>'index',$date,$hour,$profile_id,$optype_id)), null, array('class' => 'disabled'));?>
</div>